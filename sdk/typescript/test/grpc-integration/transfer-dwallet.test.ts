...
async function aliceTransferShareToBob(
	...
	const reEncryptEvent = waitForTransactionResult.Transaction?.events?.find((event) =>
		event.eventType.includes('EncryptedShareVerificationRequestEvent'),
	);
	expect(reEncryptEvent).toBeDefined();
	if (!reEncryptEvent?.bcs) throw new Error('Re-encrypt Event does not exist');
...
async function bobSignAndVerify(
	...
	const signEvent = waitForTransactionResult.Transaction?.events?.find((event) =>
