...
import {
	...
	Curve,
	grpcPrepareImportedKeyDWalletVerification,
	Hash,
	IkaGrpcClient,
	ImportedKeyDWallet,
...
import {
	...
	retryUntil,
} from '../grpc-setup/test-utils';
import { testPresign } from '../grpc-setup/dwallet-test-helpers';
import {
...
async function setupDKGFlow(
	...
	const suiClient = createTestSuiGrpcClient();
	const ikaClient = createTestIkaGrpcClient(suiClient);
	...
	if (dwalletType === 'shared') {
		...
		const dkgEvent = waitForTransactionResult.Transaction?.events?.find((event) =>
			event.eventType.includes('DWalletDKGRequestEvent'),
		);

		expect(dkgEvent).toBeDefined();
		if (!dkgEvent?.bcs) throw new Error('DKG Event does not exist');
	...
	if (dwalletType === 'imported-key') {
		...
		const importDWalletVerificationInput = await grpcPrepareImportedKeyDWalletVerification(
			ikaClient,
			curve,
			sessionIdentifier,
			signerAddress,
		...
		const result = await executeTestTransaction(suiClient, transaction, testName);
		const waitForVerificationTransactionResult = await suiClient.waitForTransaction({
			digest: result.Transaction?.digest as string,
			include: { events: true },
		});

		const verificationEvent = waitForVerificationTransactionResult.Transaction?.events?.find(
			(event) => event.eventType.includes('DWalletImportedKeyVerificationRequestEvent'),
		);
		expect(verificationEvent).toBeDefined();
		if (!verificationEvent?.bcs) throw new Error('Verification Event does not exist');
...
async function futureSignAndVerify(
	...
	const waitForFutureSignTransactionResult = await suiClient.waitForTransaction({
		digest: result1.Transaction?.digest as string,
		include: { events: true },
	});

	// Extract the partial user signature capability ID from the result using FutureSignRequestEvent
	const futureSignRequestEvent = waitForFutureSignTransactionResult.Transaction?.events?.find(
		(event) =>
	...
	if (!futureSignRequestEvent?.bcs) throw new Error('Future Sign Request Event does not exist');
	...
	const waitForSignTransactionResult = await suiClient.waitForTransaction({
		digest: result2.Transaction?.digest as string,
		include: { events: true },
	});

	const signEvent = waitForSignTransactionResult.Transaction?.events?.find((event) =>
		event.eventType.includes('SignRequestEvent'),
	);

	expect(signEvent).toBeDefined();
	if (!signEvent?.bcs) throw new Error('Sign Event does not exist');
