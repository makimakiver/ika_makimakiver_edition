...
async function requestPresignForImportedKey(
	...
	const presignEvent = waitForTransactionResult.Transaction?.events?.find((event) =>
		event.eventType.includes('PresignRequestEvent'),
	);
	expect(presignEvent).toBeDefined();
	if (!presignEvent?.bcs) throw new Error('Presign Event does not exist');
...
export async function testImportedKeyScenario(
	...
	const verificationEvent = waitForTransactionResult.Transaction?.events?.find((event) =>
	...
	const waitForSignTransactionResult = await suiClient.waitForTransaction({
		digest: signResult.Transaction?.digest as string,
		include: { events: true },
	});
