...
export class IkaGrpcClient {
	 ...
	 */
	constructor({ suiClient, config, cache = true, encryptionKeyOptions }: IkaGrpcClientOptions) {
	...
	async getDWallet(dwalletID: string): Promise<DWallet> {
		...
		return this.client.ledgerService
			.getObject({
				objectId: dwalletID,
				readMask: {
					paths: ['bcs'],
			...
			.then((obj) => {
				const dWallet = CoordinatorInnerModule.DWallet.fromBase64(grpcObjToBcs(obj));
	...
	async getPresign(presignID: string): Promise<Presign> {
		...
		return this.client.ledgerService
			...
			.then((obj) => {
				return CoordinatorInnerModule.PresignSession.fromBase64(grpcObjToBcs(obj));
			});
	}
	...
	async getEncryptedUserSecretKeyShare(
		...
		return this.client.ledgerService
			...
			.then((obj) => {
				return CoordinatorInnerModule.EncryptedUserSecretKeyShare.fromBase64(grpcObjToBcs(obj));
			});
	}
	...
	async getPartialUserSignature(
		...
		return this.client.ledgerService
			...
			.then((obj) => {
				return CoordinatorInnerModule.PartialUserSignature.fromBase64(grpcObjToBcs(obj));
			});
	}
	...
	async getSign<C extends Curve>(
		...
		const unparsedSign = await this.client.ledgerService.getObject({
			objectId: signID,
			readMask: {
				paths: ['bcs'],
			},
		...
		const sign = CoordinatorInnerModule.SignSession.fromBase64(grpcObjToBcs(unparsedSign));
	...
	async getMultipleDWallets(dwalletIDs: string[]): Promise<DWallet[]> {
		...
		return this.client.ledgerService
			.batchGetObjects({
				requests: dwalletIDs.map((id) => ({ objectId: id })),
				readMask: { paths: ['bcs'] },
			})
			.then((res) => {
				return res.response.objects.map((obj) => {
					if (obj.result.oneofKind !== 'object') {
						throw new NetworkError('Unexpected result type from batchGetObjects');
					}
					const dWallet = CoordinatorInnerModule.DWallet.fromBase64(
						grpcObjToBcs({ response: { object: obj.result.object } }),
					);
					return {
						...dWallet,
						kind: this.#getDWalletKind(dWallet),
	...
	async getOwnedDWalletCaps(
		...
		const response = await this.client.stateService.listOwnedObjects({
			owner: address,
			objectType: `${this.ikaConfig.packages.ikaDwallet2pcMpcOriginalPackage}::coordinator_inner::DWalletCap`,
		...
		return {
			dWalletCaps: response.response.objects.map((obj) =>
				CoordinatorInnerModule.DWalletCap.fromBase64(grpcObjToBcs({ response: { object: obj } })),
			),
	...
	async getActiveEncryptionKey(address: string): Promise<EncryptionKey> {
		...
		const objIDBytes = res.response.commandOutputs?.at(0)?.returnValues?.at(0)?.value?.value;
		const objID = toHex(new Uint8Array(objIDBytes ?? []));
		...
		return this.client.ledgerService
			...
			.then((obj) => CoordinatorInnerModule.EncryptionKey.fromBase64(grpcObjToBcs(obj)));
	}
	...
	async readTableVecAsRawBytes(tableID: string): Promise<Uint8Array> {
		try {
			...
			const objectIds = new Set(allTableRows.map((tableRowResult) => tableRowResult.id));
		...
		try {
			...
			for (let i = 0; i < objectIds.length; i += batchSize) {
				...
				for (const obj of batchRes.response.objects) {
					if (obj.result.oneofKind !== 'object') {
						throw new NetworkError('Failed to fetch object in batch');
					}

					const objectId = obj.result.object.objectId ?? 'unknown';
					const parsed = DynamicField(bcs.vector(bcs.u8())).fromBase64(
						grpcObjToBcs({ response: { object: obj.result.object } }),
					);
					...
					results.push(
						processor({
							...
							fields: {
								...
								value: Uint8Array.from(parsed.value),
							},
						}),
					);
				}
